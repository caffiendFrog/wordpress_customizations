        ...
        if(get_option(OPTION_RSVP_HIDE_EMAIL_FIELD) != "Y") {
          $form .= rsvp_BeginningFormField("", "rsvpBorderTop").
            ...
            RSVP_END_FORM_FIELD;
  			...
  			$form .= rsvp_buildAdditionalQuestions($a->id, $a->id, true);
...
function rsvp_buildAdditionalQuestions($attendeeID, $prefix, $isGuest=false) {
  ...
  $questions = $wpdb->get_results($sql);
	if(count($questions) > 0) {
		foreach($questions as $q) {
            ...
            if($isGuest) {
				$q->question = str_replace("you", "your guest", $q->question);
            }
...
/* SKC
	...
	$output .= RSVP_START_PARA."<input type=\"submit\" value=\"".__("RSVP", 'rsvp-plugin')."\" />".RSVP_END_PARA;
	$output .= "</form>\r\n";
	$output .= RSVP_END_CONTAINER;
	return $output;
}
  ...
  if(get_option(OPTION_RSVP_NUM_ADDITIONAL_GUESTS) != "") {
	...
	$form = "<script type=\"text/javascript\" language=\"javascript\">\r\n
                      ...
                      if(get_option(OPTION_RSVP_HIDE_EMAIL_FIELD) != "Y") {
                        ...
                        \"	<label for=\\\"newAttending\" + numAdditional + \"Email\\\">".__("Guest's email address", 'rsvp-plugin')."</label>\" + \r\n
